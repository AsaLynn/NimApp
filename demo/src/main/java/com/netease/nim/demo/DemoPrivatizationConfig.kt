 ...
 * 网易云信私有化配置项
 */
object DemoPrivatizationConfig {
    /// BASIC
    ...
    private const val YSF_DEFALUT_URL_LABEL = "ysf_defalut_url_label"
    private const val YSF_DA_URL_LABEL = "ysf_da_url_label"
    ...
    @JvmStatic
    fun saveYsfDefaultUrl(context: Context, ysfDefaultUrl: String?) {
        getSP(context).edit().putString(YSF_DEFALUT_URL_LABEL, ysfDefaultUrl).apply()
    }

    @JvmStatic
    fun saveYsfDaUrl(context: Context, ysfDaUrl: String?) {
        getSP(context).edit().putString(YSF_DA_URL_LABEL, ysfDaUrl).apply()
    }

    @JvmStatic
    fun getYsfDefalutUrlLabel(context: Context): String? {
        return getSP(context).getString(YSF_DEFALUT_URL_LABEL, null)
    }

    @JvmStatic
    fun getYsfDaUrlLabel(context: Context): String? {
    ...
    @JvmStatic
    fun checkConfigAndModifyConfig(config: String?): ServerAddresses? {
        val jsonObject = parse(config)
    ...
    private fun parseAddresses(jsonObject: JSONObject?): ServerAddresses? {
        ...
        addresses.negoKeyNeca =
            AsymmetricType.value(jsonObject.optInt(KEY_NEGO_KEY_NECA, AsymmetricType.RSA.value))
