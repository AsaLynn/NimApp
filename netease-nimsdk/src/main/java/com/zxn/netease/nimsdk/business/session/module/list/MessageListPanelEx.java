...
import com.zxn.netease.nimsdk.business.session.activity.MsgSelectActivity;
import com.zxn.netease.nimsdk.business.session.activity.VoiceTrans;
...
import com.zxn.netease.nimsdk.business.session.viewholder.robot.RobotLinkView;
...
import com.zxn.netease.nimsdk.common.ui.recyclerview.adapter.IRecyclerView;
import com.zxn.netease.nimsdk.common.ui.recyclerview.listener.OnItemClickListener;
import com.zxn.netease.nimsdk.common.ui.recyclerview.loadmore.MsgListFetchLoadMoreView;
...
import com.zxn.netease.nimsdk.common.util.sys.ClipboardUtil;
...
public class MessageListPanelEx {
    ...
    public MessageListPanelEx(Container container, View rootView, IMMessage anchor, boolean recordOnly, boolean remote, SessionCustomization customization) {
...
//            longClickItemReply(selectedItem, alertDialog, msgType);
        ...
        private void longClickItemForwardToPerson(final IMMessage item, CustomAlertDialog alertDialog) {
            alertDialog.addItem(container.activity.getString(R.string.forward_to_person), () -> {
                forwardMessage = item;
        ...
        private void longClickItemForwardToTeam(final IMMessage item, CustomAlertDialog alertDialog) {
            alertDialog.addItem(container.activity.getString(R.string.forward_to_team), () -> {
                forwardMessage = item;
                ContactSelectActivity.Option option = new ContactSelectActivity.Option();
                option.title = "选择转发的群";
        ...
        private void longClickItemDelete(final IMMessage selectedItem, CustomAlertDialog alertDialog, boolean recordOpe) {
            ...
            alertDialog.addItem(container.activity.getString(R.string.delete_has_blank) + (recordOpe ? "record" : "ignore"), () -> deleteItem(selectedItem, true, recordOpe));
        }

        // 长按菜单项--多选
        private void longClickItemMultipleSelection(IMMessage selectedItem, CustomAlertDialog alertDialog) {
            alertDialog.addItem(container.activity.getString(R.string.multiple_selection), () -> {
                // 长按消息触发，因此items不会为空
                int size = items.size();
                // 查询被长按的消息位置
    ...
    private void abovePop(View view, IMMessage item) {
        ...
        if (msgType == MsgTypeEnum.text
            ...
            mAbovePop.findViewById(R.id.tv_copy).setOnClickListener(v -> {
                ClipboardUtil.clipboardCopyText(container.activity, item.getContent());
